...
if [[ ${osvers_major} -eq 10 ]] && [[ ${osvers_minor} -eq 11 ]] && [[ ${osvers_minor} -eq 12 ]]; then
...
# This section does checking of the Mac's FileVault 2 status
# on 10.13.x
ENCRYPTION13=`fdesetup status`

if [[ ${osvers_major} -eq 10 ]] && [[ ${osvers_minor} -eq 13 ]]; then
    if [[ "$ENCRYPTION13" = "FileVault is Off." ]]; then
        echo "<result>FileVault 2 Encryption Not Enabled</result>"
    elif [[ "$ENCRYPTION13" = "FileVault is On." ]]; then
        echo "<result>FileVault 2 Encryption Complete</result>"
    else
        echo "<result>FileVault 2 Encryption Status Unknown</result>"
    fi
fi
