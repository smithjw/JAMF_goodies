...
#     Modified by:  James Smith <james@smithjw.me> 
#         Created:  2016-12-29
...
You've entered your password incorrectly five times.

Please contact the Technology team for help via Slack."
...
until dscl /Search -authonly "$CURRENT_USER" "$USER_PASS" &>/dev/null; do
    ...
    if (( TRY >= 5 )); then
        ...
        launchctl "$L_METHOD" "$L_ID" "$jamfHelper" -windowType "utility" -icon "$LOGO_PNG" -title "$PROMPT_TITLE" -description "$FORGOT_PW_MESSAGE" -button1 'OK' -button2 'Contact IT' -defaultButton 1 -timeout 30 -startlaunchd &>/dev/null &
		exit 1       
    fi
done
